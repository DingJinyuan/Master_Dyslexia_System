...
async def text_to_speech(
    ...
    # --- 缓存检查：相同 text+voice+rate 直接返回已有音频 ---
    cache_key = hashlib.md5(f"{text[:200]}{len(text)}{voice}{rate}".encode()).hexdigest()
    cached_file = os.path.join(settings.TTS_OUTPUT_DIR, f"cached_{cache_key}.mp3")
    if os.path.exists(cached_file) and os.path.getsize(cached_file) > 0:
        logger.info(f"TTS 命中缓存 {cache_key[:8]}...")
        return {
            "success": True,
            "fileId": cache_key,
            "filePath": cached_file,
            "audioUrl": f"/static/tts/cached_{cache_key}.mp3",
            "voice": voice,
            "rate": rate,
            "engine": "cached"
        }

    file_id = cache_key  # 用缓存 key 作为文件名，方便后续命中
    ...
    if success:
        ...
        return {
            ...
            "audioUrl": f"/static/tts/{filename}",
        ...
        error_msg = (
            "TTS 语音生成失败。已依次尝试 edge-tts(微软免费)、有道智云、gTTS、pyttsx3，均未成功。"
            "请检查网络连接，或确认 edge-tts 是否正常: pip install edge-tts"
        )
