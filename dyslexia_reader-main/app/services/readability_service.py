...
import logging
import jieba
from app.utils.text_utils import detect_language, preprocess_text
...
def _english_readability(text: str) -> dict:
    ...
    try:
        ...
        # 四次方根映射：学术论文 Flesch 偏低（1~20），拉高到合理区间
        import math
        adjusted = 100 * (max(flesch_score, 0.5) / 100) ** 0.25

        if adjusted >= 80: level, level_en = "非常简单", "Very Easy"
        elif adjusted >= 60: level, level_en = "简单", "Easy"
        elif adjusted >= 40: level, level_en = "中等", "Medium"
        elif adjusted >= 25: level, level_en = "较难", "Difficult"
        else: level, level_en = "非常难", "Very Difficult"

        return {
            "language": "en",
            "readabilityScore": round(adjusted, 1),
            "level": level,
            "levelEn": level_en,
            "details": {
        ...
        logger.warning("textstat 不可用，回退到简化可读性评分", exc_info=True)
